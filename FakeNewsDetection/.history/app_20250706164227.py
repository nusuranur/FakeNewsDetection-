...
if 'text_area_key' not in st.session_state:
    st.session_state.text_area_key = "news_article_input_0" # Initial unique key

inputn = st.text_area("News Article:", value=st.session_state.text_input_value, key=st.session_state.text_area_key)
...
with col2:
    ...
    if st.button("Clear Input", key="clear_input_btn"):
        ...
        st.session_state.text_area_key = str(uuid.uuid4()) # Generate a new unique key for the text area
        result_placeholder.empty() # Clear the displayed result
        st.rerun() # Rerun to reflect the cleared text area and new key
...
with st.expander("About This Detector"):
    st.markdown("""
        ...
        <p style="color: #4A2C2A;">
        It employs a <b>Logistic Regression</b> model trained on a dataset of various news articles. The text input is processed using a <b>Tf-idf Vectorizer</b> to convert text into numerical features that the model can understand.
        </p>
        <p style="color: #4A2C2A;">
